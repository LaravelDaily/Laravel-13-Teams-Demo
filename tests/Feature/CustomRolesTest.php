...
test('editor permissions include invitation:create but not team:update', function () {
    expect(TeamRole::Editor->hasPermission('invitation:create'))->toBeTrue();
    expect(TeamRole::Editor->hasPermission('team:update'))->toBeFalse();
});
...
test('members can still create and update categories', function () {
