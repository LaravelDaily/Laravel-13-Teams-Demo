...
test('hasTeamPermission returns correct values for editor', function () {
    ...
    expect($user->hasTeamPermission($team, 'category:create'))->toBeTrue();
    expect($user->hasTeamPermission($team, 'category:update'))->toBeTrue();
    expect($user->hasTeamPermission($team, 'category:delete'))->toBeFalse();
    expect($user->hasTeamPermission($team, 'post:create'))->toBeTrue();
    expect($user->hasTeamPermission($team, 'post:delete'))->toBeFalse();
});
...
test('admins have full resource permissions', function () {
    ...
    expect($user->hasTeamPermission($team, 'category:create'))->toBeTrue();
    expect($user->hasTeamPermission($team, 'category:delete'))->toBeTrue();
    expect($user->hasTeamPermission($team, 'post:create'))->toBeTrue();
    expect($user->hasTeamPermission($team, 'post:delete'))->toBeTrue();
});
