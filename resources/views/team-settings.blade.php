    ...
    <div class="mx-auto max-w-2xl space-y-10 py-8">
        ...
        <form method="POST" action="{{ route('team.settings.update') }}" class="space-y-6">
            ...
            <flux:textarea
                ...
                :value="old('description', $team->settings?->get('description', ''))"
                rows="3"
            />

            <flux:select name="timezone" :label="__('Timezone')">
                @foreach (['UTC', 'America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles', 'Europe/London', 'Europe/Berlin', 'Europe/Vilnius', 'Asia/Tokyo', 'Australia/Sydney'] as $tz)
                    <flux:select.option
                        value="{{ $tz }}"
